...
public class IMEIMaskingProviderTest extends TestLogSetUp {
  ...
  public void testMaskInvalidValue() {
    ...
    assertNull(maskingProvider.mask("foobar"));

    config.setUnspecifiedValueHandling(1);

    maskingProvider = new IMEIMaskingProvider(config);

    assertNull(maskingProvider.mask("000000000000000"));

  }

  @Test
  public void testMaskInvalidValue_returnRandom() {
    IMEIIdentifier identifier = new IMEIIdentifier();
    IMEIMaskingProviderConfig config = new IMEIMaskingProviderConfig();
  ...
  @Test
  public void testMaskInvalidValue_returnFixed() {
    IMEIMaskingProviderConfig config = new IMEIMaskingProviderConfig();
    config.setUnspecifiedValueHandling(3);
    MaskingProvider maskingProvider = new IMEIMaskingProvider(config);

    assertEquals("OTHER", maskingProvider.mask("foobar"));

    config.setUnspecifiedValueReturnMessage("unknown");
    maskingProvider = new IMEIMaskingProvider(config);

    assertEquals("unknown", maskingProvider.mask("123456789"));
  }
}
