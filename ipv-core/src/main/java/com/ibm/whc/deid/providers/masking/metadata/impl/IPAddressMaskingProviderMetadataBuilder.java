...
/**
 * Obtains metadata for the IP_ADDRESS masking provider.
 */
public class IPAddressMaskingProviderMetadataBuilder extends MaskingProviderMetadataBuilderBase {
  ...
  protected List<ConfigurationOptionModel> getOptions(MaskingProviderTypes provider, Locale locale) {
    ...
    options.add(
        buildOption(provider.getIdentifier(), "subnetsPreserve", bundle, OptionType.Integer, "0"));
    return options;
  }
}
