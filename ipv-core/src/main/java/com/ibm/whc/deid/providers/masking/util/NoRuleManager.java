...
public class NoRuleManager {
  ...
  public NoRuleManager(MaskingProviderBuilder.MaskingResource maskingResource, String resourceId, MaskingProvider noRuleResProvider) {
    this.noRuleResProvider = noRuleResProvider;
    findLeaves(maskingResource.getJsonNode(), "", maskingResource.getJsonNode(), resourceId, maskingResource.getResourceType());
  }
      ...
      if (parentNode.isValueNode()) {
        ...
        for (int i=0; i < size; i++) {
          ...
          if (!childNode.isNull()) {
            ...
            if (childNode.isArray() || childNode.isObject()) {
              ...
              map.put(new JsonNodeMapWrapper(childNode), new MaskingActionInputIdentifier(
                  noRuleResProvider, childNode, parentNode, childPath, resourceType, resourceId, rootNode));
            }
          }
        }
        ...
        while (it.hasNext()) {
          ...
          if (childNode.isNull()) {
            ...
            map.put(new JsonNodeMapWrapper(childNode), new MaskingActionInputIdentifier(noRuleResProvider, childNode, parentNode, entry.getKey(), resourceType, resourceId, rootNode));
          }
        }
      }
    }
