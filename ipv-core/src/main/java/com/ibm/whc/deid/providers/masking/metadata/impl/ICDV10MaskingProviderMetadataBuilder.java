...
/**
 * Obtains metadata for the ICDV10 masking provider.
 */
public class ICDV10MaskingProviderMetadataBuilder extends MaskingProviderMetadataBuilderBase {
  ...
  protected List<ConfigurationOptionModel> getOptions(MaskingProviderTypes provider, Locale locale) {
    ...
    options.add(buildOption(provider.getIdentifier(), "generalizeToCategory", bundle,
        OptionType.Boolean, Boolean.TRUE.toString()));
    options.add(buildOption(provider.getIdentifier(), "generalizeToChapter", bundle,
