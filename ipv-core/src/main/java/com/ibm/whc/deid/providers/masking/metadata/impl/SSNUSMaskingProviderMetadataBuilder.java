...
/**
 * Obtains metadata for the SSN_US masking provider.
 */
public class SSNUSMaskingProviderMetadataBuilder extends MaskingProviderMetadataBuilderBase {
  ...
  protected List<ConfigurationOptionModel> getOptions(MaskingProviderTypes provider, Locale locale) {
    ...
    options.add(buildOption(provider.getIdentifier(), "maskPreserveAreaNumber", bundle,
        OptionType.Boolean, Boolean.TRUE.toString()));
    options.add(buildOption(provider.getIdentifier(), "maskPreserveGroup", bundle,
        OptionType.Boolean, Boolean.TRUE.toString()));
    return options;
  }
}
