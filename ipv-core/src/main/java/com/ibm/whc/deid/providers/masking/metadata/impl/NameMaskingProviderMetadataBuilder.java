...
/**
 * Obtains metadata for the NAME masking provider.
 */
public class NameMaskingProviderMetadataBuilder extends MaskingProviderMetadataBuilderBase {
  ...
  protected List<ConfigurationOptionModel> getOptions(MaskingProviderTypes provider, Locale locale) {
    ...
    options.add(buildOption(provider.getIdentifier(), "maskingAllowUnisex", bundle,
        OptionType.Boolean, Boolean.FALSE.toString()));
    options.add(buildOption(provider.getIdentifier(), "maskGenderPreserve", bundle,
