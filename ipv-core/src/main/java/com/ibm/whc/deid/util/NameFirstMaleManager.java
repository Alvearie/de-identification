...
public class NameFirstMaleManager extends LocalizedResourceManager<FirstName> {
  ...
  public static NameFirstMaleManager buildNameFirstMaleManager(String localizationProperty) {
    ...
    try {
      Collection<ResourceEntry> resourceEntries = LocalizationManager
          .getInstance(localizationProperty).getResources(Resource.FIRST_NAME_MALE);
  ...
  protected static void loadCSVRecord(String fileName, String countryCode,
      NameFirstMaleManager manager, CSVRecord record) {
    try {
   ...
   * @throws RuntimeException if any of the data in the record is invalid for its target purpose.
   */
  protected static void loadRecord(String countryCode, NameFirstMaleManager manager,
      String... record) {
